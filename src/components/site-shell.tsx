...
export const nav = [
  ...
  ["Contact", "/contact"], ["Log In", "/login"], ["Enroll", "/enroll"],
];
...
export function SiteFooter() {
  return <footer><div className="footer-top"><Link href="/" className="footer-wordmark">MALEX</Link><p>A trusted place to learn,<br/>belong, and become.</p><Link href="/enroll" className="circle-link">Apply<br/>now ↗</Link></div><div className="footer-grid"><div><strong>Malex International School</strong><p>Enugu, Nigeria<br/>+234 800 MALEX SCHOOL<br/>hello@malexschool.edu.ng</p></div><div>{nav.slice(0,3).map(([a,b])=><Link href={b} key={b}>{a}</Link>)}</div><div>{nav.slice(3).map(([a,b])=><Link href={b} key={b}>{a}</Link>)}</div><div><Link href="/login">Log In</Link><Link href="/contact">Book a school visit</Link></div></div><div className="copyright">© {new Date().getFullYear()} Malex International School <span>Privacy · Safeguarding · Accessibility</span></div></footer>;
}
